...
export function MobileAppsSection({ onDemoClick }: { onDemoClick?: () => void }) {
  const { t } = useI18n()
  ...
  // Set up auto-scrolling for mobile and tablet
  useEffect(() => {
    if ((isMobile || isTablet) && scrollContainerRef.current) {
      // Start auto-scrolling after 3 seconds
      const timeout = setTimeout(() => {
        const interval = setInterval(() => {
          const scrollContainer = scrollContainerRef.current
          if (scrollContainer) {
            // If we can't scroll right anymore, reset to the beginning
            if (!canScrollRight) {
              scrollContainer.scrollTo({ left: 0, behavior: "smooth" })
            } else {
              // Otherwise, scroll right by a small amount
  ...
  const resumeAutoScroll = () => {
    if ((isMobile || isTablet) && !autoScrollInterval) {
      const interval = setInterval(() => {
        const scrollContainer = scrollContainerRef.current
        if (scrollContainer) {
          if (!canScrollRight) {
  ...
  return (
    <SectionBackground id="mobile-apps" ref={containerRef} className="bg-gradient-to-b from-white to-slate-50">
      ...
      >
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-800 mb-4 md:mb-6">{t("mobile.title")}</h2>
        <p className="text-lg md:text-xl text-slate-600 max-w-3xl mx-auto">{t("mobile.subtitle")}</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-center">
        ...
        >
          <div className="mb-8">
            <h3 className="text-2xl font-bold text-slate-800 mb-4 md:mb-6">{t("mobile.appsInPractice")}</h3>
          ...
          <div className="mb-8">
            <h3 className="text-2xl font-bold text-slate-800 mb-4">{t("mobile.customerBenefits")}</h3>
          ...
          <div className="mb-8">
            <h3 className="text-2xl font-bold text-slate-800 mb-4">{t("mobile.businessBenefits")}</h3>
      ...
      >
        <div className="inline-block bg-blue-50 rounded-full px-4 py-1.5 text-sm text-blue-700 font-medium mb-4 border border-blue-100">
          <Search className="inline-block h-4 w-4 mr-1 -mt-0.5" /> {t("mobile.searchVisible")}
        </div>
        <h3 className="text-2xl font-bold text-slate-800 mb-4">{t("mobile.appInSearchResults")}</h3>
